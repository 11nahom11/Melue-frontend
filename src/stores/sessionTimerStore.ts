// Shared session timer store (lives outside React).
//
// The 90-minute block timer must keep counting real time even when the
// Session screen unmounts (e.g. the user switches to the Dashboard tab and
// back). Keeping it in component state resets it to the full duration on
// every remount, so we track a single wall-clock state here instead.
//
// All times are epoch milliseconds (Date.now()) so backgrounded JS timers
// can't drift the countdown.
