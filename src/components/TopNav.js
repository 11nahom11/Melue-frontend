...
//
// ⚠️ SHARED APP CHROME — this nav bar (logo, tab links, user name + logout)
// wraps every screen in the Figma, including screens outside this
// module's scope (Assessments, ABC Log). It most likely belongs with
// whoever builds the app shell (MR-6 Admin Panel Shell). Included here so
// the Daily Operations screens are
// demoable end-to-end, but this should be reconciled with / replaced by the
// real shared shell once that exists — don't let two versions drift apart.
...
const TABS = ['Dashboard', 'Session', 'Assessments', 'Daily Notes', 'ABC Log', 'Scheduling', 'Attendance', 'Parents'];

export default function TopNav({ activeTab, onTabPress, userName = 'Teacher A', role = 'Teacher', onLogout }) {
  return (
