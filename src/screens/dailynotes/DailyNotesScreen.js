...
export default function DailyNotesScreen({ navigation }) {
  const { logout } = useAuth();
  const [search, setSearch] = useState('');
  const [records, setRecords] = useState([]);
  const [summary, setSummary] = useState(null);
  const [stats, setStats] = useState({ sessionsCompleted: 0, totalTrials: 0, avgIndependence: 0, reviewsPending: 0 });
...
function SummaryRow({ label, value, bold, color }) {
  return (
    <View style={styles.summaryRow}>
      ...
      <Text style={[bold ? typography.bodyBold : typography.body, color && { color }]}>{value}</Text>
    </View>
  );
}

const DEMO_STATS = { sessionsCompleted: 6, totalTrials: 124, avgIndependence: 68, reviewsPending: 1 };
const DEMO_RECORDS = [
