...
function AttachmentRow({ attachment, onRemove }) {
  const iconByType = { image: 'image', video: 'video', pdf: 'file-text', other: 'paperclip' };
  return (
...
export default function SessionNoteEditorScreen({ route, navigation }) {
  const sessionId = route?.params?.sessionId ?? 'DEMO_SESSION_ID';
  const mode = route?.params?.mode || 'edit'; // 'edit' | 'view'
