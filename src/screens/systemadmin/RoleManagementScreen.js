...
function RoleFormModal({ visible, role, onClose, onSave }) {
...
export default function RoleManagementScreen({ navigation }) {
  ...
  const handleSave = async (payload) => {
    try {
      if (payload.id) { await updateRole(payload.id, payload); setRoles((prev) => prev.map((r) => (r.id === payload.id ? { ...r, ...payload } : r))); }
      else { const { data } = await createRole(payload); setRoles((prev) => [...prev, data]); }
    } catch (err) {
      if (payload.id) setRoles((prev) => prev.map((r) => (r.id === payload.id ? { ...r, ...payload } : r)));
