...
export default function IupGenerationScreen({ navigation }) {
  const [candidates, setCandidates] = useState([]);
  const [selectedStudentId, setSelectedStudentId] = useState(null);
  const [context, setContext] = useState(null);
  const [goalBank, setGoalBank] = useState([]);
  const [slots, setSlots] = useState({ station1: [null, null], station2: [null, null] });
  const [selectorTarget, setSelectorTarget] = useState(null); // { station, slotIndex }
  ...
  const handleRemoveGoal = (station, slotIndex) => {
