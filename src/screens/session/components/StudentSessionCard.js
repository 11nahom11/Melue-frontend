// screens/session/components/StudentSessionCard.js
...
// student: {
//   id, name, initial, program, active,
//   goals: [{ id, name, category, goalType: 'standard' | 'task_analysis', steps?: [...] }],
//   trials: [{ promptLevel, timestamp }]
// }
          ...
          <View style={styles.statsHeaderRow}>
            ...
            {student.trials?.length > 0 && (
              ...
              <Text style={typography.caption}>
                {student.trials.filter((t) => t.promptLevel === 'INDEPENDENT').length}/{student.trials.length} independent · {Math.round((student.trials.filter((t) => t.promptLevel === 'INDEPENDENT').length / student.trials.length) * 100)}% accuracy
              </Text>
            )}
          </View>
          <View style={styles.trialsBox}>
            {student.trials?.length ? (
              <View style={styles.trialIconsRow}>
                {student.trials.slice(0, 5).map((t, i) => (
                  <View
                    key={i}
                    style={[
                      styles.trialIcon,
...
function TaskAnalysisStepList({ goal, disabled, onStepPrompt, onPressAnywhere }) {
