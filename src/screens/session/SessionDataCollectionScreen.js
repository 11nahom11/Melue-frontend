...
// TODO: replace with real navigation params (sessionId comes from
// tapping "Start Session" on the Dashboard's Today's Schedule card - MR-39).
export default function SessionDataCollectionScreen({ route, navigation }) {
  const sessionId = route?.params?.sessionId ?? 'DEMO_SESSION_ID';
  const { logout } = useAuth();

  const [loading, setLoading] = useState(true);
  const [session, setSession] = useState(null); // { teacherName, stationName, roomName, blockDurationMinutes, students: [] }
  const [secondsRemaining, setSecondsRemaining] = useState(null);
  const [isRunning, setIsRunning] = useState(true); // spec shows a green play/pause toggle next to the timer
  const [incidentModal, setIncidentModal] = useState(null); // { studentId, studentName, goalName }
  ...
  const handleSelectPromptLevel = async (studentId, goalId, level, stepId) => {
  ...
  const handleOpenIncidentModal = (studentId, goalId) => {
  ...
  const handleCancelIncident = (hadChanges) => {
    if (hadChanges) {
  ...
  const handleSaveIncident = async (incidentData) => {
    try {
      await recordIncident(sessionId, incidentModal.studentId, incidentData);
    } catch (err) {
  ...
  const handleMasteryCheck = (studentId, goalId) => {
    // Real flow per SCR-004: navigate to the Goal Mastery Check screen,
    // don't just fire an API call.
    navigation?.navigate?.('GoalMasteryCheck', { studentId, goalId });
  };
  ...
  const handleViewGoalProgress = (studentId, goalId) => {
