...
function AssessmentRow({ label, status, progress, onPress }) {
  const s = STATUS_STYLE[status] || STATUS_STYLE['Not Started'];
  return (
...
export default function AssessmentDashboardScreen({ navigation }) {
          ...
          {data.students.map((s) => (
            <View key={s.id} style={styles.studentCard}>
              ...
              <View style={styles.studentBtnRow}>
                <TouchableOpacity style={styles.primaryLaunchBtn}>
                  <Text style={styles.primaryLaunchBtnText}>ABLLS Assessment →</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.secondaryLaunchBtn}>
...
const DEMO_DATA = {
  ...
  students: [
    { id: 'student-a', name: 'Student A', initial: 'S', age: 6, program: 'Regular Program', ablls: { status: 'In Progress', progress: 45 }, behavior: { status: 'Not Started', progress: 0 } },
    { id: 'student-b', name: 'Student B', initial: 'S', age: 7, program: 'Regular Program', ablls: { status: 'Completed', progress: 100 }, behavior: { status: 'Completed', progress: 100 } },
    { id: 'student-c', name: 'Student C', initial: 'S', age: 5, program: 'Pooled-Out', ablls: { status: 'In Progress', progress: 20 }, behavior: { status: 'In Progress', progress: 20 } },
    { id: 'student-d', name: 'Student D', initial: 'S', age: 6, program: 'Regular Program', ablls: { status: 'Not Started', progress: 0 }, behavior: { status: 'Not Started', progress: 0 } },
  ],
};
