...
const PROMPT_COLOR = { INDEPENDENT: '#22C55E', G: '#EAB308', PP: '#F97316', FP: '#EF4444' };

function TrialLogModal({ visible, goalName, trials, onClose }) {
  return (
...
function GoalSummaryRow({ goal, onViewTrialLog }) {
...
function StudentSummaryCard({ student, expanded, onToggle, onViewTrialLog }) {
  return (
...
export default function SessionSummaryScreen({ route, navigation }) {
  ...
  const [trialLogTarget, setTrialLogTarget] = useState(null); // { goalName, trials }
  ...
  const load = useCallback(async () => {
    try {
      ...
      setSummary(data);
      setExpandedStudentIds(Object.fromEntries(data.students.map((s) => [s.id, true])));
    } catch (err) {
