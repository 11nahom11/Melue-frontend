...
const STATUS_FLOW = ['scheduled', 'confirmed', 'checked_in', 'in_progress', 'completed'];
const STATUS_LABEL = {
  scheduled: 'Scheduled',
  confirmed: 'Confirmed',
...
export default function AppointmentFormModal({
  ...
  const handleCancel = () => {
    Alert.alert('Cancel appointment?', 'This cannot be undone.', [
      ...
      { text: 'Cancel Appointment', style: 'destructive', onPress: () => onCancelAppointment?.(appointment.id) },
    ]);
  };
  ...
  const handleMarkMissed = () => {
    Alert.alert('Mark as No Show?', undefined, [
      ...
      { text: 'Mark No Show', style: 'destructive', onPress: () => onMarkStatus?.(appointment.id, 'no_show') },
    ]);
  };
