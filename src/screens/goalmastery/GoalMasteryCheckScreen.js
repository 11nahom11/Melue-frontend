...
function TeacherVerificationSection({ title, teacherName, outcome, onOutcomeChange, promptUsed, onPromptChange, notes, onNotesChange }) {
  return (
...
export default function GoalMasteryCheckScreen({ route, navigation }) {
  const { studentId = 'DEMO_STUDENT', goalId = 'DEMO_GOAL' } = route?.params || {};

  const [data, setData] = useState(null); // { studentName, goalName, station, primaryTeacherSummary, teacherB: {name}, teacherC: {name}, status }
  const [teacherBOutcome, setTeacherBOutcome] = useState(null);
  const [teacherBPrompt, setTeacherBPrompt] = useState(null);
  const [teacherBNotes, setTeacherBNotes] = useState('');
  const [teacherCOutcome, setTeacherCOutcome] = useState(null);
  const [teacherCPrompt, setTeacherCPrompt] = useState(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setData((prev) => ({ ...prev, status: 'pending_approval' }));
