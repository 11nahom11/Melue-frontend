...
function TemplateEditorModal({ visible, template, onClose, onSave }) {
  ...
  const updateStep = (id, description) => setSteps((prev) => prev.map((s) => (s.id === id ? { ...s, description } : s)));
...
export default function TaskAnalysisTemplatesScreen({ navigation }) {
  ...
  const handleSave = async (payload) => {
    try {
      const { data } = await saveTaskAnalysisTemplate(payload.id, payload);
