...
export default function GoalDomainDefinitionsScreen({ navigation }) {
  ...
  const handleUpdateName = (id, name) => setDomains((prev) => prev.map((d) => (d.id === id ? { ...d, name } : d)));
  const handleUpdateDesc = (id, description) => setDomains((prev) => prev.map((d) => (d.id === id ? { ...d, description } : d)));
