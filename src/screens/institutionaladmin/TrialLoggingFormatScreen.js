...
export default function TrialLoggingFormatScreen({ navigation }) {
  ...
  const handleUpdateLabel = (id, label) => setLevels((prev) => prev.map((l) => (l.id === id ? { ...l, label } : l)));
  const handleUpdateColor = (id, color) => setLevels((prev) => prev.map((l) => (l.id === id ? { ...l, color } : l)));
