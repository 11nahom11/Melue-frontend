...
export default function InstitutionalAdminNav({ sectionTitle, breadcrumb, scrCode }: InstitutionalAdminNavProps) {
  const { logout } = useAuth();
  return (
    <View style={s.wrap}>
      <View>
        <Text style={s.title}>{sectionTitle ?? 'Admin Panel'}</Text>
        <Text style={s.breadcrumb}>{breadcrumb ?? 'Clinical Configuration'}</Text>
      </View>
      <View style={s.right}>
