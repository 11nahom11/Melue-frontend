...
// The Director-side receiving end of Teacher's SCR-004 (Goal Mastery
// Check) - Teacher B/C verification comes here for final approval.
...
export default function GoalMasteryApprovalScreen({ navigation }) {
  ...
  const handleViewDetail = async (goalId) => {
    try {
  ...
  const handleApprove = async (goalId, notes) => {
  ...
  const handleReject = async (goalId, reason, notes) => {
