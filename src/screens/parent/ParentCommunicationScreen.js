// screens/parent/ParentCommunicationScreen.js
...
export default function ParentCommunicationScreen({ navigation }) {
  ...
  const handleSend = async () => {
    ...
    const newMsg = { id: `local-${Date.now()}`, sender: 'parent', senderLabel: 'You', roleTag: null, text: draft };
    setThread((prev) => [...prev, newMsg]);
    setDraft('');
    try { await sendParentMessage(activeId, { text: newMsg.text }); } catch (err) {}
  };
...
const DEMO_THREAD = [
  { id: '1', sender: 'team', senderLabel: 'Teacher A', roleTag: 'Teacher', text: 'Student A had a great session today!', timestamp: '10:15 AM' },
];
